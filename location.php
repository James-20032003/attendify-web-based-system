...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $designatedLat = 0.327096; // Latitude of UICT
    $designatedLon = 32.614729; // Longitude of UICT
    ...
    // Check if user coordinates are provided
    if ($userLat !== null && $userLon !== null) {
        ...
        if ($distance <= $radius) {
            ...
            $stmt = $db->prepare("INSERT INTO checkin (username, date, time) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $username, $date, $time);

            if ($stmt->execute()) {
                echo "You are at UICT. Your Arrival Details captured.";
            ...
            echo "You are not at UICT. Please go to the Campus. The System cannot capture your Arrival Details.";
        }
    } else {
        echo "Coordinates are missing. Please provide valid latitude and longitude.";
    }
} else {
